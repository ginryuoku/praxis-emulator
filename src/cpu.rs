...
impl CPU {
    pub fn new(boot_rom: Vec<u8>) -> CPU {
        CPU {
    ...
    pub fn run(&mut self, clock_ticks: usize, frame_count: usize) {
        println!("Executing frame: {}. is_halted: {}", frame_count, self.is_halted);
        let mut frame_cycles = 0;
        while !self.is_halted && frame_cycles <= clock_ticks {
